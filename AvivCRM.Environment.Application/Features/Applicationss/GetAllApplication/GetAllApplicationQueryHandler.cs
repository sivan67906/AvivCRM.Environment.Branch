...
internal class GetAllApplicationQueryHandler(IApplication _applicationRepository, IMapper mapper) : IRequestHandler<GetAllApplicationQuery, ServerResponse>
{
    public async Task<ServerResponse> Handle(GetAllApplicationQuery request, CancellationToken cancellationToken)
    {
        ...
        if (application is null) return new ServerResponse(Message: "No Application Found");
        ...
        return new ServerResponse(IsSuccess: true, Message: "List of Applications", Data: applicationResponse);
    }
}