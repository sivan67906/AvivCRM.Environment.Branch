...
internal class UpdateAttendanceSettingCommandHandler(IValidator<UpdateAttendanceSettingRequest> _validator, IAttendanceSetting _attendanceSettingRepository, IUnitOfWork _unitOfWork, IMapper mapper) : IRequestHandler<UpdateAttendanceSettingCommand, ServerResponse>
{
    public async Task<ServerResponse> Handle(UpdateAttendanceSettingCommand request, CancellationToken cancellationToken)
    {
        // Validate Request
        ...
        return new ServerResponse(IsSuccess: true, Message: "AttendanceSetting updated successfully", Data: attendanceSettingEntity);
    }
}