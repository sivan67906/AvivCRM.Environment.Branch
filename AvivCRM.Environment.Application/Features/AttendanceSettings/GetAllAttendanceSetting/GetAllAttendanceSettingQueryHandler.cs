...
internal class GetAllAttendanceSettingQueryHandler(IAttendanceSetting _attendanceSettingRepository, IMapper mapper) : IRequestHandler<GetAllAttendanceSettingQuery, ServerResponse>
{
    public async Task<ServerResponse> Handle(GetAllAttendanceSettingQuery request, CancellationToken cancellationToken)
    {
        ...
        if (attendanceSetting is null) return new ServerResponse(Message: "No AttendanceSetting Found");
        ...
        return new ServerResponse(IsSuccess: true, Message: "List of AttendanceSettings", Data: attendanceSettingResponse);
    }
}