    ...
    IMapper mapper) : IRequestHandler<UpdateNotificationMainCommand, ServerResponse>
{
    ...
    {
        // Validate Request
        FluentValidation.Results.ValidationResult validate = await _validator.ValidateAsync(request.NotificationMain);
        if (!validate.IsValid)
        {
        ...
        NotificationMain? notificationMain = await _notificationMainRepository.GetByIdAsync(request.NotificationMain.Id);
