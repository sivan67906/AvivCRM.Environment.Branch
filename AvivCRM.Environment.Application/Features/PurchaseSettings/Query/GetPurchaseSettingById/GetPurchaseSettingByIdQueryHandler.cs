    ...
    : IRequestHandler<GetPurchaseSettingByIdQuery, ServerResponse>
{
    public async Task<ServerResponse> Handle(GetPurchaseSettingByIdQuery request, CancellationToken cancellationToken)
    {
        ...
        Domain.Entities.PurchaseSetting? purchaseSetting = await purchaseSettingRepository.GetByIdAsync(request.Id);
        ...
        GetPurchaseSetting? purchaseSettingResponse = mapper.Map<GetPurchaseSetting>(purchaseSetting); // <DTO> (parameter)
