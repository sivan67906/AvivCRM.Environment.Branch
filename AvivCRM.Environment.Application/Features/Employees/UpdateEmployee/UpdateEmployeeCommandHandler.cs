...
internal class UpdateEmployeeCommandHandler(IValidator<UpdateEmployeeRequest> _validator, IEmployee _employeeRepository, IUnitOfWork _unitOfWork, IMapper mapper) : IRequestHandler<UpdateEmployeeCommand, ServerResponse>
{
    public async Task<ServerResponse> Handle(UpdateEmployeeCommand request, CancellationToken cancellationToken)
    {
        ...
        return new ServerResponse(IsSuccess: true, Message: "Employee updated successfully", Data: employeeEntity);
    }
}