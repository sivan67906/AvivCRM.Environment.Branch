...
internal class GetAllClientQueryHandler(IClient _clientRepository, IMapper mapper) : IRequestHandler<GetAllClientQuery, ServerResponse>
{
    public async Task<ServerResponse> Handle(GetAllClientQuery request, CancellationToken cancellationToken)
    {
        ...
        if (client is null) return new ServerResponse(Message: "No Client Found");

        // Map the Client to the response
        var clientResponse = mapper.Map<IEnumerable<GetClient>>(client);
        if (clientResponse is null) return new ServerResponse(Message: "Client Not Found");

        return new ServerResponse(IsSuccess: true, Message: "List of Clients", Data: clientResponse);
    }
}