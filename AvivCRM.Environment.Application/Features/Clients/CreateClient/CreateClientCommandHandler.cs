...
internal class CreateClientCommandHandler(IValidator<CreateClientRequest> _validator, IClient _clientRepository, IUnitOfWork _unitOfWork, IMapper mapper) : IRequestHandler<CreateClientCommand, ServerResponse>
{
    public async Task<ServerResponse> Handle(CreateClientCommand request, CancellationToken cancellationToken)
    {
        ...
        return new ServerResponse(IsSuccess: true, Message: "Client created successfully", Data: clientEntity);
    }
}
