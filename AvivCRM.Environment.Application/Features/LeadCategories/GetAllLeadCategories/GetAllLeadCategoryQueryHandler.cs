...
internal class GetAllLeadCategoryQueryHandler(ILeadCategory _leadCategoryRepository, IMapper mapper) : IRequestHandler<GetAllLeadCategoryQuery, ServerResponse>
{
    public async Task<ServerResponse> Handle(GetAllLeadCategoryQuery request, CancellationToken cancellationToken)
    {
        ...
        if (leadCategory is null) return new ServerResponse(Message: "No Lead Category Found");
